package ru.tinkoff.kora.techempower.loom.undertow.undertow.request;
...
public class UndertowPublicApiRequest implements PublicApiRequest {
    ...
    private HttpBodyInput getContent(HttpServerExchange exchange) throws IOException {
        ...
        if (!exchange.isInIoThread()) {
            return new ru.tinkoff.kora.http.server.undertow.request.UndertowRequestHttpBody(context, exchange, null);
        }
        try (var pooled = exchange.getConnection().getByteBufferPool().allocate()) {
            ...
            if (res == 0) {
                ...
                prefetched.add(data);
                return new ru.tinkoff.kora.http.server.undertow.request.UndertowRequestHttpBody(context, exchange, prefetched);
            }
